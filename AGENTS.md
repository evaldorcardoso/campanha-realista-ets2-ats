...
- **Combobox de cidade (busca):** os 7 campos de cidade (`cfgCity`, `cgFrom`, `cgTo`, `rpFrom`, `rpTo`, `exCity`, `cfCity`) são combobox custom (sem biblioteca) — `<div class="city-combo">` com `<input class="city-combo-input">` (mesmo `id` de antes) + `<ul class="city-combo-list">`. O valor é lido via `.value` do input (texto da cidade, sem estado), então a lógica existente não muda. O widget usa **delegação global de eventos** (`document.addEventListener`) em `app.js` — não anexe handlers por elemento nem recrie `<select>`; `populateCitySelect(inputEl, selectedValue, game)` guarda as opções em `inputEl._comboOpts` (`{value,label}`) e seta `inputEl._comboVal`. Busca sem acentos; **blur restaura** a última seleção válida ou esvazia (sem texto livre, igual ao select nativo). Ao mexer no widget, atualizar junto o CSS em `styles.css` (`.city-combo*`).
